...
| 无障碍焦点管理| 严格遵守规定键盘焦点 (Focus) 的设计规范，规定所有可交互元素（按钮、链接、输入框）必须有清晰、高对比度的 :focus-visible 样式，并实现焦点陷阱 (Focus Trap) 逻辑，确保在使用 Tab 键时体验完美。 |
| 无障碍设计   | 严格遵循 WCAG 2.1 标准，无键盘焦点无障碍、无障碍焦点管理、无障碍颜色方案、无障碍文本大小调整                                                                  |
| 响应式图片 (LCP)| 懒加载、图片质量优化、图片质量优先级（大图优先、小图后） , 集成或推荐使用 Hexo Filter 或 Gulp 任务来生成 <picture> 或 srcset 属性，以确保在不同设备上加载最合适的图片资源。           |          
| 错误处理   | 错误处理页面（404、500）、错误处理动画（Lottie）, 定义一个统一的**“空状态 (Empty State)”**组件样式，内容应包含：友好的提示文本、返回或重置筛选的按钮，并适配明暗主题。                 |
| 交互式反馈（Haptic/Audio)| 在移动端主题切换或关键操作（如点赞/收藏）时，利用 Web APIs（如 Vibration API）提供**轻量级振动反馈**（需谨慎使用，以防过度打扰）。                                      |
| 兼容性       | 兼容所有主流浏览器（Chrome、Firefox、Safari、Edge、Opera）、移动端（iOS、Android）、平板电脑, 不同尺寸的PC设备,无障碍设备（屏幕阅读器、无障碍软件）                      |
...
|  搜索框  | 渐变背景（`#f97316→#fbbf24`），文字白色 12px                          | 渐变背景（`#ea580c→#f59e0b`），文字白色 12px                          | 无差异，字号统一 12px                                                |


#### 功能实现
1. 文章搜索实现支持： 
  - Hexo Generator Search: 生成 JSON 文件，前端 IndexDB + Fuse.js 搜索。
  - Algolia / Google Custom Search: 依赖第三方 API。
  - 同时，需设计搜索结果页/搜索 Modal的毛玻璃样式和动效。
  - 需要维护好索引与内存占用优化
...
#### 功能实现
1. 评论功能实现支持：
   - 支持多种评论服务（ Waline、Disqus、Giscus，Artalk,twikoo,utterance,gitalk,valine,睿言云评 等）及其配置方法。主题需提供一个统一的评论组件接口。
   - 并定义其组件在明暗模式下的样式适配规则。建议提供一个降级/默认的 Hexo 内置评论模板（如果用户不配置任何系统）。
...
1. **Hexo 主题目录初始化**：
   ...
   - 建议将主题配置进行模块化拆分，利用 Hexo 的 theme_config 或其他方案，允许用户通过多个小的 YAML 文件来配置，例如 _config/i18n.yml, _config/social.yml。
